...
const TrafficChart = () => {
    ...
    const calculateInitialAvgTimes = (data) => {
        const initialAvgTimes = {
            '대전': (parseInt(data.csudj) + parseInt(data.cdjsu)) / 2,
            '대구': (parseInt(data.csudg) + parseInt(data.cdgsu)) / 2,
            '울산': (parseInt(data.csuus) + parseInt(data.cussu)) / 2,
            '부산': (parseInt(data.csubs) + parseInt(data.cbssu)) / 2,
            '광주': (parseInt(data.csugj) + parseInt(data.cgjsu)) / 2,
            '목포': (parseInt(data.csump) + parseInt(data.cmpsu)) / 2,
            '강릉': (parseInt(data.csukr) + parseInt(data.ckrsu)) / 2,
            '남양주': (parseInt(data.csuyy) + parseInt(data.cyysu)) / 2,
        };
    ...
    return (
        <div>
            <h5 style={{ textAlign: 'center' }}>서울에서 주요 도시까지의 소요 시간</h5>
